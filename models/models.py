...
class index_custom_cnam(models.Model):
    ...
    grouping_date = fields.Date("Date du regroupement", store=True, default=lambda self: self._set_default_grouping_date())
...
class ExamRoom(models.Model):
    ...
    def compute_state(self):
        ...
        for room in self:
            ...
            room.state = 'busy' if regrouping and regrouping.numbers_student > 0 else 'partially' if regrouping else 'free'
...
class UnitEnseignementConfig(models.Model):
    ...
    formation_ids = fields.Many2many("training.edu", string="Formations", default=lambda self: self._get_default_formation_ids())
